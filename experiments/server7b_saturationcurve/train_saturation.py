...
Frequency pairs: 16→32, 32→64, 64→128   |   Grid: 512×512

PURPOSE
-------
Determine the operating dataset size N* before running the full hyperparameter
search (Experiment 2). N* is the knee of the validation RelL2 curve: the point
at which doubling the data no longer halves the error.
...
USAGE
-----
  python train_saturation.py                       # full run
  python train_saturation.py --fast                # smoke test: N ∈ {20, 50, 100}
  python train_saturation.py --n 150 300 600       # custom N list
  python train_saturation.py --device cuda         # force device

OUTPUT  (all paths relative to the directory this file lives in)
------
  results/
    datasets/           — cached .npz files (reusable across runs)
    run_<timestamp>/
      saturation_curve.json           — all metrics, all N, all pairs
      plot_saturation_curve.png       — RelL2 vs N (per pair + mean)
      plot_convergence_curves.png     — train/val per epoch, per N
      plot_per_pair_breakdown.png     — per-pair RelL2 at each N
      plot_trivial_baseline.png       — model vs identity-mapping baseline
      checkpoints/                    — best model per N (model_N{n}.pt)
...
def build_helmholtz_matrix(n: int, omega: float, k: float, dx: float,
    ...
    σ(x) = eta · ((npml - i)/npml)²  inside the PML layer (quadratic ramp).
    """
...
def _make_fourier_channels(n: int, k_bands: int = 6) -> np.ndarray:
    """
    Fourier positional encoding: sin/cos at 2^k·π frequency, for both x and y.
    Returns (4·k_bands, n, n).  K=6 → 24 channels.
    Highest frequency 2^5=32 cycles/unit resolves λ_min ≈ 0.03,
    sufficient for all four ω values including ω=128.
    """
...
# Input channel count: Re + Im + 24 Fourier + PML + ω + η = 29
N_INPUT_CHANNELS = 29
...
def generate_sample(omega_low: float, omega_high: float,
                    n_sources: int, rng: np.random.Generator) -> dict:
    """
    Draw n_sources sources with random positions, amplitudes, and phases.
    Solve Helmholtz at omega_low (input) and omega_high (target) with the
    same source configuration.

    Returns
    -------
    dict with keys: u_low, u_high, source_field, omega_low, omega_high
      - source_field is the complex RHS f used for both solves
      - All arrays are complex128, shape (GRID_N, GRID_N)
    """
    ...
    u_low  = solve_helmholtz(omega_low,  K, source_field, NPML, ETA[int(omega_low)])
    u_high = solve_helmholtz(omega_high, K, source_field, NPML, ETA[int(omega_high)])
...
def sample_to_tensor(sample: dict) -> tuple:
    """
    Convert a raw sample dict to (input_tensor, target_tensor, source_real).

    Input tensor  (29, 512, 512):
      ch  0     Re(u_low)
      ch  1     Im(u_low)
      ch  2–25  Fourier positional encoding (K=6, 4 channels per band)
      ch  26    PML map (0→1 ramp)
      ch  27    ω_low  / 128  (normalised constant field)
      ch  28    η_low  / 200  (normalised constant field)

    Target tensor (2, 512, 512):
      ch  0     Re(u_high)
      ch  1     Im(u_high)

    source_real (512, 512):   Re(f) — the true RHS, used by the residual loss.
    """
    u_low   = sample["u_low"].astype(np.complex64)
    u_high  = sample["u_high"].astype(np.complex64)
    omega_l = float(sample["omega_low"])
    eta_l   = ETA[int(omega_l)] / 200.0     # max η=180 → 0.9
    ...
    source_re  = sample["source_field"].real.astype(np.float32)
...
class HelmholtzDataset(Dataset):
    ...
    source_real is the true Re(f) used for the PDE residual loss.
    """
    ...
    def generate(cls, n_per_pair: int, seed: int = GLOBAL_SEED,
                 verbose: bool = True) -> "HelmholtzDataset":
        """
        Generate n_per_pair samples for each of the 3 frequency pairs
        (balanced). Returns a HelmholtzDataset with 3·n_per_pair samples.
        """
        rng = np.random.default_rng(seed)
        inputs, targets, sources = [], [], []
        total = 3 * n_per_pair

        for pair_idx, (omega_low, omega_high) in enumerate(FREQ_PAIRS):
            for i in range(n_per_pair):
                if verbose:
                    done = pair_idx * n_per_pair + i + 1
                    print(f"\r  Generating {done}/{total}  "
                          f"[{omega_low}→{omega_high}  sample {i+1}/{n_per_pair}]",
                          end="", flush=True)

                n_src  = rng.integers(3, 7)     # S ~ Uniform{3,4,5,6}
                sample = generate_sample(omega_low, omega_high, n_src, rng)
                inp, tgt, src = sample_to_tensor(sample)
                inputs.append(inp)
                targets.append(tgt)
                sources.append(src)
        ...
        return cls(inputs, targets, sources)
...
class DilatedConvBlock(nn.Module):
    """Conv2d → InstanceNorm2d → Activation. Padding preserves spatial size."""
...
class FrequencyTransferCNN(nn.Module):
    """
    Flat (no downsampling) CNN for Helmholtz frequency transfer.
    Spatial resolution is preserved at 512×512 throughout.

    Architecture: stem (1×1) → depth × DilatedConvBlock → head (1×1, linear)
    Linear dilation: rates 1, 2, 3, ..., depth.
    """
    ...
    def __init__(self, in_channels=N_INPUT_CHANNELS, out_channels=2,
        ...
        # NO activation on output — physical values are unbounded
...
def interior_mask(n=GRID_N, npml=NPML, device=torch.device("cpu")):
    """Boolean mask (1,1,n,n): True inside the (n-2·npml)² interior."""
...
def mse_interior(pred, target, mask):
    """MSE of the real channel (ch 0) in the interior only."""
    m    = mask[0, 0]
...
def rel_l2_interior(pred, target, mask):
    """Relative L2 of the real channel in the interior only."""
    m     = mask[0, 0]
    diff  = pred[:, 0][:, m]   - target[:, 0][:, m]
    denom = target[:, 0][:, m]
...
def imag_mse_interior(pred, target, mask):
    """MSE of the imaginary channel (ch 1) in the interior — diagnostic only."""
    m    = mask[0, 0]
    diff = pred[:, 1][:, m] - target[:, 1][:, m]
    return (diff**2).mean()


def helmholtz_residual_loss(pred, source_real, omega_high, mask):
    """
    PDE residual loss on the real channel in the interior.

    Computes (Δ + k²ω²) Re(û) using a 5-point FD Laplacian and
    compares to Re(f) — the true source field.

    Parameters
    ----------
    pred        : (B, 2, n, n)
    source_real : (B, n, n)  — Re(f), the true RHS (NOT Re(u_low))
    omega_high  : scalar or (B,) tensor — target frequency per sample
    mask        : (1, 1, n, n) interior mask
    """
    import torch.nn.functional as F

    dx   = 1.0 / (INTERIOR - 1)
    u_re = pred[:, 0:1, :, :]   # (B, 1, n, n)
    ...
    # omega_high may be a scalar or a (B,) tensor; broadcast to (B,1,1,1)
    if isinstance(omega_high, torch.Tensor):
        om = omega_high.view(-1, 1, 1, 1).to(pred.device)
    else:
        om = float(omega_high)
...
class CombinedLoss(nn.Module):
    """
    L = λ1·MSE + λ2·RelL2 + λ3·Residual
    All on the real channel, interior only (PML weight = 0).

    λ3 is warmed up linearly from 0 over warmup_epochs to avoid
    destabilising gradients when the prediction is initially poor.
    """
    ...
    def forward(self, pred, target, source_real, omega_high):
        mask = self.mask.to(pred.device)

        l_mse = mse_interior(pred, target, mask)
        l_rel = rel_l2_interior(pred, target, mask)
        l_res = helmholtz_residual_loss(pred, source_real, omega_high, mask)
        l_im  = imag_mse_interior(pred, target, mask)   # diagnostic only
...
def _omega_high_from_batch(inp: torch.Tensor) -> torch.Tensor:
    """
    Extract ω_high per sample from the input tensor.
    ch 27 = ω_low / 128.  For our three pairs the mapping is:
      16/128 = 0.125 → ω_high = 32
      32/128 = 0.25  → ω_high = 64
      64/128 = 0.5   → ω_high = 128
    Returns a float tensor of shape (B,) with the actual ω_high value.
    """
    omega_low_norm = inp[:, 27, 0, 0]          # (B,)
    omega_low      = omega_low_norm * 128.0
    omega_high     = omega_low * 2.0            # all pairs are octave doublings
    return omega_high
...
def train_one_epoch(model, loader, optimiser, loss_fn, device):
    ...
    for inp, tgt, src in loader:
        ...
        # Extract correct ω_high per sample — used by the residual loss
        omega_high = _omega_high_from_batch(inp).to(device)

        optimiser.zero_grad()
        pred   = model(inp)
        losses = loss_fn(pred, tgt, src, omega_high)
...
def evaluate_full(model, loader, device):
    """
    Full evaluation pass: per-pair RelL2, MSE, imaginary MSE.

    Returns
    -------
    dict with keys:
      rel_l2          : overall mean (fraction)
      mse             : overall mean
      imag_mse        : overall mean
      per_pair        : {pair_str: {rel_l2, mse, imag_mse}}  e.g. "16→32"
    """
    model.eval()
    ...
    for inp, tgt, src in loader:
        ...
        omega_low_norm = inp[:, 27, 0, 0].cpu().numpy()   # (B,)
        omega_lows     = (omega_low_norm * 128.0).round().astype(int)
        ...
        for b in range(inp.shape[0]):
            ...
            rl2     = (p_re - t_re).norm() / (t_re.norm() + 1e-8)
            mse_re  = ((p_re - t_re)**2).mean()
            mse_im  = ((p_im - t_im)**2).mean()

            pair_data[pk]["rel_l2"].append(rl2.item())
            pair_data[pk]["mse"].append(mse_re.item())
            pair_data[pk]["imag_mse"].append(mse_im.item())
...
def trivial_baseline(loader, device):
    """
    Compute RelL2 of the trivial prediction: û = u_low (no transfer).
    This is the floor any real model must beat.
    """
    mask = interior_mask(device=device)
    m    = mask[0, 0]
    pair_rl2 = {f"{lo}→{hi}": [] for lo, hi in FREQ_PAIRS}

    for inp, tgt, _ in loader:
        inp, tgt = inp.to(device), tgt.to(device)
        omega_low_norm = inp[:, 27, 0, 0].cpu().numpy()
        omega_lows     = (omega_low_norm * 128.0).round().astype(int)

        for b in range(inp.shape[0]):
            ol = omega_lows[b]; oh = ol * 2; pk = f"{ol}→{oh}"
            if pk not in pair_rl2:
                continue
            pred_re = inp[b, 0][m]   # Re(u_low) as trivial prediction
            true_re = tgt[b, 0][m]
            rl2 = (pred_re - true_re).norm() / (true_re.norm() + 1e-8)
...
def train_to_convergence(dataset, device, checkpoint_path,
                         max_epochs=200, patience=15,
                         batch_size=4, lr=1.1e-4, lambda3=0.1,
                         verbose=True):
    """
    Train the fixed architecture (Optuna winner) to convergence.
    Saves best model weights to checkpoint_path.

    Returns
    -------
    dict: best_val_rel_l2, epochs_trained, best_epoch,
          train_curve, val_curve, val_per_pair_curves,
          trivial_baseline_per_pair, final_eval
    """
    n_total = len(dataset)
    ...
    val_mse_curve   = []
    val_im_curve    = []
    # per-pair val RelL2 curves: list of dicts
...
def run_saturation_curve(n_values, run_dir, device, fast=False):
    """
    For each N in n_values:
      1. Load or generate balanced dataset (3·N samples, cached to disk)
      2. Train to convergence, save best checkpoint
    ...
    for n_per_pair in n_values:
        print(f"\n{'='*64}")
        print(f"  N per pair = {n_per_pair}  (total = {3*n_per_pair})")
        print(f"{'='*64}")

        cache = dataset_dir / f"dataset_N{n_per_pair}_seed{GLOBAL_SEED}.npz"
        if cache.exists():
            print(f"  Loading cached dataset: {cache.name}")
            dataset = HelmholtzDataset.load(cache)
        else:
            print(f"  Generating {3*n_per_pair} samples...")
            dataset = HelmholtzDataset.generate(n_per_pair, seed=GLOBAL_SEED)
            dataset.save(cache)
            print(f"  Saved → {cache}")
        ...
        result    = train_to_convergence(
            dataset, device, ckpt_path,
            max_epochs=max_ep, patience=pat, batch_size=4, verbose=True,
        )
...
def plot_saturation_curve(results, run_dir):
    """
    Plot 1: RelL2 vs N — per pair + mean + trivial baseline + thresholds.
    """
    ...
    # Collect per-pair best val RelL2 and trivial baseline
    ...
    fig.suptitle("Experiment 1 — Data Saturation Curve\n"
                 "Multi-Source Helmholtz Frequency Transfer  (ω: 16→32, 32→64, 64→128)",
                 fontweight="bold", fontsize=12)

    for pk in pair_keys:
        ax.plot(n_vals, pp_rl2[pk], "o-",
                color=PAIR_COLORS[pk], lw=2, ms=7, label=f"Model  {pk}")
        ax.plot(n_vals, pp_trivial[pk], "s--",
                color=PAIR_COLORS[pk], lw=1, ms=5, alpha=0.5,
                label=f"Trivial {pk}")
    ...
    ax.set_title("Validation/Test RelL2 vs Dataset Size", **STYLE)
...
def plot_convergence_curves(results, run_dir):
    """
    Plot 2: Train vs Val RelL2 curves per epoch, one subplot per N.
    Also shows imaginary MSE as a secondary diagnostic.
    """
    details = results["details"]
    ...
    fig, axes = plt.subplots(n_rows, n_cols,
                              ...
                              squeeze=False)
    fig.suptitle("Experiment 1 — Convergence Curves per N\n"
                 "Train (dashed) vs Validation RelL2 (solid)  +  Imaginary MSE (dotted)",
    ...
    for idx, d in enumerate(details):
        ...
        ax.plot(epochs, [v*100 for v in d["val_curve"]],
                color="#2E6DA4", lw=2, label="Val RelL2")
        ax.plot(epochs, [v*100 for v in d["train_curve"]],
                color="#2E6DA4", lw=1.5, ls="--", alpha=0.6, label="Train RelL2")

        ax2.plot(epochs, d["val_imag_mse_curve"],
                 color="#9B59B6", lw=1, ls=":", alpha=0.7, label="Im MSE")
...
def plot_per_pair_breakdown(results, run_dir):
    """
    Plot 3: Side-by-side bars showing RelL2 per frequency pair at each N.
    Gives a clear view of which pair is hardest and whether they converge together.
    """
    details   = results["details"]
    n_vals    = results["n_values"]
    pair_keys = [f"{lo}→{hi}" for lo, hi in FREQ_PAIRS]
    ...
    fig.suptitle("Experiment 1 — Per-Pair RelL2 Breakdown (Test Set)",
                 fontweight="bold", fontsize=12)
    ...
    for col, pk in enumerate(pair_keys):
        ...
        ax.bar(xs - 0.2, rl2_vals, width=0.35,
               color=PAIR_COLORS[pk], alpha=0.85, label="Model")
        ax.bar(xs + 0.2, triv_vals, width=0.35,
               color=PAIR_COLORS[pk], alpha=0.3, label="Trivial baseline")

        ax.axhline(THRESH_MIN,    color="#E07B39", ls="--", lw=1.2)
        ax.axhline(THRESH_STRONG, color="#2CA02C", ls="--", lw=1.2)
        ax.set_xticks(xs)
        ax.set_xticklabels([str(n) for n in n_vals], rotation=30, fontsize=9)
        ax.set_title(f"ω: {pk}", fontsize=11, fontweight="bold",
...
def plot_per_pair_convergence(results, run_dir):
    """
    Plot 4: Val RelL2 per epoch, coloured by pair, one subplot per N.
    Shows whether all three pairs converge at the same rate or one lags.
    """
    details   = results["details"]
    pair_keys = [f"{lo}→{hi}" for lo, hi in FREQ_PAIRS]
    ...
    fig, axes = plt.subplots(n_rows, n_cols,
                              figsize=(5 * n_cols, 4 * n_rows),
                              squeeze=False)
    fig.suptitle("Experiment 1 — Per-Pair Validation RelL2 Convergence",
                 fontweight="bold", fontsize=11)
        ...
        for pk in pair_keys:
            ...
            if curve:
                ax.plot(epochs, [v*100 for v in curve],
                        color=PAIR_COLORS[pk], lw=1.8, label=pk)

        _add_thresholds(ax)
        ax.axvline(d["best_epoch"], color="grey", ls=":", lw=1,
                   label=f"Best epoch ({d['best_epoch']})")
...
def plot_trivial_baseline(results, run_dir):
    """
    Plot 5: Model improvement ratio = trivial_baseline / model_rl2 per pair.
    A ratio > 1 means the model beats the trivial. Shows how much is gained.
    """
    details   = results["details"]
    n_vals    = results["n_values"]
    pair_keys = [f"{lo}→{hi}" for lo, hi in FREQ_PAIRS]
    ...
    fig.suptitle("Experiment 1 — Improvement Over Trivial Baseline\n"
    ...
    for pk in pair_keys:
        ratios = []
        for d in details:
            model_rl2  = d["test_eval"]["per_pair"].get(pk, {}).get("rel_l2", float("nan"))
            triv_rl2   = d["trivial_baseline"]["per_pair"].get(pk, float("nan"))
            ratio      = triv_rl2 / model_rl2 if model_rl2 > 0 else float("nan")
            ratios.append(ratio)
        ax.plot(n_vals, ratios, "o-",
                color=PAIR_COLORS[pk], lw=2, ms=7, label=pk)

    ax.axhline(1.0, color="black", ls="--", lw=1.2, label="Trivial baseline (ratio=1)")
...
# ══════════════════════════════════════════════════════════════════════════════
#  SECTION 8 — JSON + SUMMARY TEXT
# ══════════════════════════════════════════════════════════════════════════════

def _estimate_knee(n_vals, mean_rl2):
    """
    Estimate N* as the point of maximum marginal gain reduction.
    Returns (n_star, knee_idx).
    """
    if len(mean_rl2) < 3:
...
def save_json(results, run_dir):
    """Save all metrics to saturation_curve.json."""
    pair_keys = [f"{lo}→{hi}" for lo, hi in FREQ_PAIRS]
...
def save_summary(results, json_data, run_dir):
    """Write human-readable summary.txt including N* recommendation."""
    pair_keys = [f"{lo}→{hi}" for lo, hi in FREQ_PAIRS]
    ...
    for d in results["details"]:
        ...
        if vrl2 < THRESH_STRONG / 100:
            flag = "  ✓ STRONG"
        elif vrl2 < THRESH_MIN / 100:
            flag = "  ✓ MIN"
        lines.append(
            f"{d['n_per_pair']:>8}  "
            f"{vrl2*100:>8.2f}%  "
    ...
    lines += [
        ...
        "",
        "DECISION RULES",
        "  Val RelL2 still above 20% at N=2400  → data-starved; check solver quality",
...
def main():
    parser = argparse.ArgumentParser(
        description="Experiment 1 — Data Saturation Curve"
    )
    ...
    # ── output directory: timestamped run inside results/ ───────────────────
    ...
    print(f"Datasets cached: {RESULTS_DIR / 'datasets'}")
    print(f"N values:        {n_values}")
    print(f"Input channels:  {N_INPUT_CHANNELS}  (Re, Im, Fourier×24, PML, ω, η)")
    print(f"Architecture:    width=128  depth=8  kernel=7  dilation=linear\n")

    # ── run ─────────────────────────────────────────────────────────────────
    results = run_saturation_curve(n_values, run_dir, device, fast=args.fast)
