#!/bin/bash
# Three-seed left-metric sensitivity for one frequency-pair variant.

#SBATCH --job-name=pml_freq_left
#SBATCH --output=/home/fkiewiet/Freq2Transfer/experiments/claude/eigenvalue_1d/pml_1d/sbatch_logs/job32_%x_%j.out
#SBATCH --error=/home/fkiewiet/Freq2Transfer/experiments/claude/eigenvalue_1d/pml_1d/sbatch_logs/job32_%x_%j.err
...
echo "Left-metric eval $TAG variant=$VARIANT ckpt=$CKPT"
for SEED in 2025 1111 3333; do
  python measure_pml_left_metric.py \
    --ckpt "$CKPT" \
    --config "$BASE/pml_config.json" \
    --seed "$SEED" \
    --out "$BASE/left_metric_${RESULT_TAG}_seed${SEED}.json"
done
