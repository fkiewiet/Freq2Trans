#!/usr/bin/env bash
# ══════════════════════════════════════════════════════════════════════════════
# wave7b_N4800_final.sh — N=4800 final run, UP direction, λ_imag=1.0
# Run on: wave7b.mit.edu
#
...
#   ssh wave7b.mit.edu
#   cd ~/Freq2Transfer
#   bash experiments/claude/launch/wave7b_N4800_final.sh
# ══════════════════════════════════════════════════════════════════════════════
set -euo pipefail
...
SRC_DS="${REPO_ROOT}/experiments/claude/datasets/up_N4800_seed42"
LOCAL_DS="/tmp/freq2t_up_N4800_seed42"
...
LOG="${LOG_DIR}/wave7b_final_up_N4800_${TS}.log"

mkdir -p "${LOG_DIR}" "${RESULTS_DIR}"

[[ -d "${SRC_DS}" ]] || { echo "ERROR: source dataset not found: ${SRC_DS}"; exit 1; }

WRAPPER="${LOG_DIR}/wave7b_final_up_N4800_${TS}.sh"
...
# ── Step 2: train ─────────────────────────────────────────────────────────────
echo "=== final_up_N4800 started: \$(date) ==="
time ${PYTHON} ${SCRIPT} \\
    --direction   ${DIRECTION} \\
    --n           4800 \\
    --dataset     ${LOCAL_DS} \\
...
echo ""
echo "[wave7b_N4800_final.sh] Job launched."
