#!/bin/bash
# Three-seed left-residual metric sensitivity for one beta=0.3 PML variant.

#SBATCH --job-name=pml_arch_left
#SBATCH --output=/home/fkiewiet/Freq2Transfer/experiments/claude/eigenvalue_1d/pml_1d/sbatch_logs/job27_%x_%j.out
#SBATCH --error=/home/fkiewiet/Freq2Transfer/experiments/claude/eigenvalue_1d/pml_1d/sbatch_logs/job27_%x_%j.err
...
echo "Job 27: beta=0.3 left-residual metric sensitivity"
echo "variant=$VARIANT ckpt=$CKPT"

for SEED in 2025 1111 3333; do
  python measure_pml_left_metric.py \
    --ckpt "$CKPT" \
    --config "$BASE/pml_config.json" \
    --seed "$SEED" \
    --out "$BASE/left_metric_${TAG}_seed${SEED}.json"
done
