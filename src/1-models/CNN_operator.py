...
class FlatBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dilation=1):
        super().__init__()
        # Padding is calculated to maintain grid size (N=512)
        # Formula: padding = (kernel_size - 1) / 2 * dilation
        padding = 3 * dilation 
        self.conv = nn.Conv2d(
            in_channels, 
            out_channels, 
...
class FlatOperator(nn.Module):
    ...
    def __init__(self, in_channels=25):
        """
        Args:
            in_channels (int): 25 channels as follows:
                               1 (RHS) + 1 (PML) + 20 (Fourier) + 
                               1 (Omega) + 1 (Direction) + 2 (Coords).
        """
        super(FlatOperator, self).__init__()
        
        # Architecture logic based on Logbook #03 for multi-scale field resolution
        ...
        self.layer4 = FlatBlock(64, 128, dilation=8) # Bottleneck expansion
...
# --- UTILITIES (Outside the class to fix Pylance errors) ---

def get_model_summary(model, input_size=(25, 512, 512)):
    """
    Utility to check parameter count and output shapes.
    Requires: pip install torchsummary
    """
    try:
