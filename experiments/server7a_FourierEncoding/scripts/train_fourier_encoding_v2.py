...
BASE_DIR = Path(__file__).resolve().parent.parent.parent / "results" / "fourier_study_long"
...
OMEGA = 64.0
ETA = 50.0           
EPOCHS = 2000        # Extended for high-fidelity convergence
LR = 0.00011
HIDDEN_DIM = 128
LAYERS = 8
...
def run_experiment(mode, device):
    print(f"\n>>> Starting Fourier Study (Long Run): {mode.upper()}")
    model = PhysicsOperator(mode).to(device)
    optimizer = optim.AdamW(model.parameters(), lr=LR)
    
    # 1. Learning Rate Scheduler: Reduces LR by half every 500 steps
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % 100 == 0 or epoch == 1:
            ...
            print(f"Epoch {epoch}/{EPOCHS} | Loss: {loss.item():.2e} | LR: {curr_lr:.2e}")
...
if __name__ == "__main__":
    ...
    plt.yscale('log')
    plt.title(fr"2000-Epoch Fourier Efficiency (Grid 512, PML {PML_THICKNESS}px, $\omega$={OMEGA})")
    plt.xlabel("Training Epochs")
    plt.ylabel("Mean Squared Error (Log Scale)")
    plt.legend()
    plt.grid(True, which="both", alpha=0.3)
    
    save_path = BASE_DIR / "comparison_summary_2000.png"
    plt.savefig(save_path, dpi=300)
    print(f"\n[SUCCESS] Long-run plot saved to: {save_path}")