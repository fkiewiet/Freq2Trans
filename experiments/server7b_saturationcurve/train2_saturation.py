"""
train2_saturation.py
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
EXPERIMENT 1 — DATA SATURATION CURVE  (bidirectional)
...
NEW vs train_saturation.py
--------------------------
  --direction up    (default) — identical to original: low→high transfer
  --direction down            — high→low transfer: 32→16, 64→32, 128→64

  Scientific note: upward transfer asks the model to hallucinate finer
  oscillations from coarser input (harder). Downward transfer asks it to smooth
  fine structure into coarser output (expected to be easier). If downward RelL2
  is not lower, that is a finding worth reporting.

  Results are written to separate timestamped directories so up/down runs
  never overwrite each other. Datasets are also cached separately
  (dataset_N{n}_seed{s}_up.npz vs dataset_N{n}_seed{s}_down.npz).
...
USAGE
-----
  python train2_saturation.py                        # full up run
  python train2_saturation.py --direction down       # full down run
  python train2_saturation.py --fast                 # smoke test N ∈ {20, 50, 100}
  python train2_saturation.py --n 150 300 600        # custom N list
  python train2_saturation.py --direction down --fast

OUTPUT  (relative to this file)
------
  results/
    datasets/           — cached .npz files (reusable)
    run_up_<ts>/        — up-direction run outputs
    run_down_<ts>/      — down-direction run outputs
...
# Input channels: Re + Im + 24 Fourier + PML + ω + η = 29
N_INPUT_CHANNELS = 29
...
def generate_sample(omega_in: float, omega_out: float,
    ...
    Solve Helmholtz at omega_in (input) and omega_out (target) with the
    same source configuration.

    Works for both directions:
      up:   omega_in < omega_out  (e.g. 16→32)
      down: omega_in > omega_out  (e.g. 32→16)

    Returns
    -------
    dict with keys: u_low (= u_in), u_high (= u_out), source_field,
                    omega_low (= omega_in), omega_high (= omega_out)
    The keys u_low/u_high/omega_low/omega_high are kept for compatibility
    with the tensor builder; their meaning is now "input" and "target"
    regardless of which is physically larger.
    """
    ...
    return {
        "u_low":        u_in,       # input field  (may be high ω in down mode)
        "u_high":       u_out,      # target field (may be low ω in down mode)
        "source_field": source_field,
        "omega_low":    omega_in,   # ω of input tensor
        "omega_high":   omega_out,  # ω of target tensor
    }
...
def sample_to_tensor(sample: dict) -> tuple:
    ...
    Input tensor  (29, 512, 512):
      ...
      ch  27    ω_input  / 128  (normalised constant field)
      ch  28    η_input  / 200  (normalised constant field)
    ...
    Normalisation: fields are divided by the RMS of Re(u_input) over the
    interior. This brings all frequency pairs to roughly unit scale,
    preventing the high-ω fields from dominating the loss.
    """
    ...
    # ── Per-sample normalisation by RMS of input interior ──────────────────
    interior = slice(NPML, NPML + INTERIOR)
    rms = float(np.sqrt(np.mean(np.abs(u_low[interior, interior])**2))) + 1e-8
    u_low  = u_low  / rms
    u_high = u_high / rms   # same factor preserves the transfer ratio
...
class HelmholtzDataset(Dataset):
    ...
    def generate(cls, n_per_pair: int, seed: int = GLOBAL_SEED,
        ...
        """
        Generate n_per_pair samples for each pair in FREQ_PAIRS (balanced).
        Uses the global FREQ_PAIRS, so direction must be set before calling.
        Total samples = len(FREQ_PAIRS) × n_per_pair.
        """
        rng = np.random.default_rng(seed)
        inputs, targets, sources = [], [], []
        total = len(FREQ_PAIRS) * n_per_pair
            ...
            for i in range(n_per_pair):
                if verbose:
                    ...
                    print(f"\r  Generating {done}/{total}  "
                          f"[{omega_in}→{omega_out}  sample {i+1}/{n_per_pair}]",
                          end="", flush=True)

                n_src  = rng.integers(3, 7)
                sample = generate_sample(omega_in, omega_out, n_src, rng)
...
def helmholtz_residual_loss(pred, source_real, omega_target, mask):
    ...
    Computes (Δ + k²ω²) Re(û) and compares to Re(f).

    omega_target is the ω of the *output* field — the equation the
    ...
    if isinstance(omega_target, torch.Tensor):
        om = omega_target.view(-1, 1, 1, 1).to(pred.device)
    else:
        om = float(omega_target)
...
class CombinedLoss(nn.Module):
    ...
    Interior only, real channel. λ3 warmed up over warmup_epochs.
    """
...
def _omega_target_from_batch(inp: torch.Tensor,
                             direction: str = "up") -> torch.Tensor:
    """
    Extract ω_target (the output frequency) per sample from the input tensor.

    ch 27 = ω_input / 128.
    For upward transfer   (direction="up"):   ω_target = ω_input × 2
    For downward transfer (direction="down"): ω_target = ω_input / 2

    Returns float tensor of shape (B,) with actual ω values.
    """
    omega_input_norm = inp[:, 27, 0, 0]          # (B,)
    omega_input      = omega_input_norm * 128.0
    if direction == "up":
        return omega_input * 2.0
    else:
        return omega_input / 2.0
...
def evaluate_full(model, loader, device, direction):
    ...
    Pair labels are derived from ch27 and the transfer direction.
    """
    model.eval()
    ...
    for inp, tgt, src in loader:
        ...
        omega_in_norm = inp[:, 27, 0, 0].cpu().numpy()
        omega_ins     = (omega_in_norm * 128.0).round().astype(int)
...
def trivial_baseline(loader, device, direction):
    """
    Compute RelL2 of the trivial prediction û = u_input (no transfer).
    For up: predicting the low-ω field as the high-ω field.
    For down: predicting the high-ω field as the low-ω field.
    """
    mask = interior_mask(device=device)
    m    = mask[0, 0]
...
def train_to_convergence(dataset, device, checkpoint_path, direction,
                         ...
                         verbose=True):
    """
    Train the fixed architecture to convergence with early stopping.
...
def run_saturation_curve(n_values, run_dir, device, direction, fast=False):
    """
    ...
    for n_per_pair in n_values:
        ...
        # Direction-tagged cache so up/down datasets never collide
        cache = (dataset_dir /
...
def _direction_label(direction):
    ...
    return "ω: 32→16, 64→32, 128→64  (downward)"
...
def plot_per_pair_breakdown(results, run_dir):
    ...
    fig.suptitle(
        f"Experiment 1 — Per-Pair RelL2 Breakdown (Test Set)  [{direction.upper()}]",
        fontweight="bold", fontsize=12)
...
def plot_per_pair_convergence(results, run_dir):
    ...
    fig, axes = plt.subplots(n_rows, n_cols,
                              figsize=(5*n_cols, 4*n_rows), squeeze=False)
    fig.suptitle(
        f"Experiment 1 — Per-Pair Validation RelL2 Convergence  [{direction.upper()}]",
        fontweight="bold", fontsize=11)
...
def save_summary(results, json_data, run_dir):
    ...
    direction_note = (
        "Scientific note: upward transfer (low→high ω) is generally harder;\n"
        "  the model must infer finer oscillations from coarser input.\n"
        "  Downward transfer (high→low ω) is expected to show lower RelL2.\n"
        "  If this is not observed, that is a reportable finding."
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Experiment 1 — Data Saturation Curve (bidirectional)"
    )
    ...
    # ── set global FREQ_PAIRS based on direction ─────────────────────────────
    global FREQ_PAIRS
    ...
    # ── output directory: direction-tagged + timestamped ─────────────────────
    ...
    print(f"Direction:       {args.direction}  "
          f"({_direction_label(args.direction)})")
    ...
    # ── run ──────────────────────────────────────────────────────────────────
    results = run_saturation_curve(n_values, run_dir, device,
                                   args.direction, fast=args.fast)
